...
const PlanCard: React.FC<{ plan: Plan }> = ({ plan }) => {
  ...
  const cycleLabel =
    billingOptions.find((opt) => opt.id === selectedCycle)?.label ?? "Mensal";
  const dynamicWhatsappLink = generateWhatsAppLink(
    plan.name,
  ...
  return (
    <div
      className={`bg-slate-800 p-8 rounded-lg w-full h-full flex flex-col border-2 ${plan.featured ? "border-blue-500 shadow-lg shadow-blue-500/20" : "border-slate-700"}`}
    >
      <h3 className="text-2xl font-bold text-center text-blue-400 uppercase tracking-wider">
        {plan.name}
      </h3>
      <p className="text-center text-slate-400 mt-2 mb-6 h-12">
        {plan.idealFor}
      </p>
...
const singleSportPlansData: { [key: string]: Plan } = {
  Corrida: {
    ...
    idealFor:
      "Para atletas que buscam superar seus limites no asfalto ou na trilha.",
    prices: {
      monthly: { price: "R$ 250" },
...
const aquathlonPlan: Plan = {
  name: "Aquathlon Performance",
  idealFor:
    "A preparação completa para os desafios de natação e corrida combinados.",
  prices: {
    monthly: { price: "R$ 380" },
  ...
  buttonText: "ESCOLHER PLANO AQUATHLON",
  featured: true,
};
  ...
  return (
    <div className="bg-slate-800 p-8 rounded-lg w-full flex flex-col border-2 border-blue-500 shadow-lg shadow-blue-500/20 h-full">
      <div className="flex border-b border-slate-700 mb-6">
        ...
        <div className="py-3 px-4 sm:px-6 font-bold text-lg text-slate-600 cursor-not-allowed">
          Ciclismo{" "}
          <span className="text-xs font-normal align-middle">(em breve)</span>
        </div>
      </div>
    ...
    <section id="plans" className="py-20 bg-slate-900">
      <div className="container mx-auto px-6 text-center">
        <h2 className="text-4xl font-bold mb-4 text-white">
          Escolha o plano que te levará ao{" "}
          <span className="text-blue-500">próximo nível</span>
        </h2>
        <p className="text-blue-300 bg-blue-500/10 inline-block px-4 py-1 rounded-full mb-12">
